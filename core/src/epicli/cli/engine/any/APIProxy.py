...
class APIProxy:
    ...
    def get_ips_for_feature(self, component_key, use_public_ips=None):
        ...
        if hasattr(component_config, 'machines'):
            for machine in component_config.machines:
                machine_doc = select_first(self.config_docs, lambda x: x.kind == 'infrastructure/machine' and x.name == machine)
